  ...
  return (
    <>
      <nav className="fixed top-0 left-0 right-0 z-50 h-[88px] flex items-center justify-around md:justify-between px-4 sm:px-12 border-b border-gold/30 bg-dark-bg/90 backdrop-blur-md text-cream">
        ...
        >
          <div className="font-cinzel font-bold text-lg md:text-xl tracking-wider bg-gradient-to-r from-gold-bright via-saffron to-gold bg-clip-text text-transparent leading-none text-center">
            Agarwal Pujan Bhandar
          </div>
          <span className="font-sanskrit text-[10px] md:text-xs text-gold/80 block tracking-widest mt-1 text-center">
            Ashish Agarwal
          </span>
        </div>
        ...
        <ul className="hidden lg:flex mx-3 items-center gap-10 font-cormorant text-base md:text-lg">
          ...
          <li className="hover:text-gold-bright cursor-pointer hover:underline underline-offset-4 decoration-saffron transition"><Link to="/product">Products</Link></li>
        ...
        <div className="flex items-center gap-2 sm:gap-6">
          ...
          <div className="hidden md:block w-48 md:w-64">
            <RenderSearchBar innerRef={desktopSearchRef} />
          </div>

          {/* Wishlist */}
          <button className="relative p-2 border border-gold/30 rounded-full hover:bg-gold/10 text-gold transition cursor-pointer" title="Wishlist">
          ...
          {/* Cart */}
          <button className="relative p-2 border border-gold/30 rounded-full bg-gold/10 hover:bg-gold/20 text-gold-bright transition cursor-pointer" title="Shopping Basket">
            <ShoppingCart className="w-4 h-4" />
            <span className="absolute -top-1.5 -right-1.5 w-4 h-4 bg-saffron text-[9px] font-sans text-white rounded-full flex items-center justify-center border border-gold/40 animate-pulse">
              5
            </span>
          </button>
          ...
          <button 
            className="p-2 border border-gold/30 rounded-full hover:bg-gold/10 text-gold transition cursor-pointer"
            title="Profile"
            onClick={() => alert("Profile feature coming soon!")}
          >
            <User className="w-4 h-4" /> {/* Make sure to add User to your lucide-react import at the top! */}
          </button>

          {/* Mobile Menu Icon Toggle */}
          <button 
            onClick={() => setMobileMenuOpen(!mobileMenuOpen)} 
            className="lg:hidden p-2 text-gold hover:text-gold-bright transition"
          >
      ...
      {mobileMenuOpen && (
        <div className="fixed top-[88px] left-0 right-0 z-40 lg:hidden flex flex-col items-center bg-dark-bg/95 border-b border-gold/20 backdrop-blur-lg p-6 animate-fadeIn">
          ...
          <div className="w-full max-w-sm mb-6 md:hidden">
            <RenderSearchBar innerRef={mobileSearchRef} />
          </div>
          ...
          <ul className="w-full text-center flex flex-col gap-5 font-cormorant text-xl text-cream">
            ...
            <li onClick={() => setMobileMenuOpen(false)} className="hover:text-gold-bright py-1"><Link to="/product">Products</Link></li>
